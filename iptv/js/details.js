...
async function initializeMoviePage(movieId) {
    try {
        ...
        const [movie, trailer, credits] = await Promise.all([
            ...
            fetchApi(`${TMDB_BASE_URL}/movie/${movieId}/credits?api_key=${TMDB_API_KEY}&language=pt-BR`) // <-- ADICIONA ESTA LINHA
        ]);
        // Guarda o ID do IMDb (necessário para o player de filmes)
        currentImdbId = movie.imdb_id;

        // Renderiza os detalhes
        renderCommonDetails(movie, trailer, credits); // <-- PASSA OS CRÉDITOS
...
async function initializeSeriesPage(tvId) {
    try {
        ...
        const [show, trailer, credits] = await Promise.all([
            ...
            fetchApi(`${TMDB_BASE_URL}/tv/${tvId}/credits?api_key=${TMDB_API_KEY}&language=pt-BR`) // <-- ADICIONA ESTA LINHA
        ]);
        
        // Renderiza detalhes comuns (título, poster, etc.)
        renderCommonDetails(show, trailer, credits); // <-- PASSA OS CRÉDITOS
...
function renderCommonDetails(data, videoData, credits) {
    ...
    // ... (depois do código do Trailer)

    // Set cast
    if (credits && credits.cast) {
        const castList = document.getElementById('cast-list');
        castList.innerHTML = credits.cast
            .slice(0, 8) // Limita a 8 membros
            .map(actor => {
